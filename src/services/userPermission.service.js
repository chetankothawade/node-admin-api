...
export const userPermissionService = {
  toModuleIdList(roleModules = []) {
    return roleModules
      .map((row) => Number(row.moduleId ?? row.module_id))
      .filter((id) => Number.isInteger(id) && id > 0);
  },

  async resolveUser(userIdentifier, attributes = { id: true, role: true }) {
    ...
    const user = await userPermissionRepository.findUser(where, attributes);
  ...
  async resolveModulePermissionId(payload) {
    const directId = Number(payload.modulePermissionId ?? payload.module_permission_id);
    ...
    const permissionId = Number(payload.permissionId ?? payload.permission_id);
  ...
  async toggle(userIdOrPayload, modulePermissionIdArg, isCheckedArg) {
    ...
    const user = await this.resolveUser(payload.userId ?? payload.user_id, { id: true });
  ...
  async getModulePermissionMatrix(userId, isPermission = "N") {
    ...
    const modulePermissions = await userPermissionRepository.findModulePermissions({ raw: true });
    const userPermissions = await userPermissionRepository.findUserPermissions({
      where: { userId: user.id },
      attributes: ["modulePermissionId"],
      raw: true,
    });

    const userPermissionIds = userPermissions.map((p) => p.modulePermissionId);

    const modulesResult = modules.map((m) => {
      const perms = permissions.map((p) => {
        const mp = modulePermissions.find(
          (x) => x.moduleId === m.id && x.permissionId === p.id
        );
      ...
      return {
        ...
        seq_no: m.seqNo,
        seqNo: m.seqNo,
        is_sub_module: m.isSubModule,
        isSubModule: m.isSubModule,
        parent_id: m.parentId,
        parentId: m.parentId,
        is_permission: m.isPermission,
  ...
  async getUserModuleAccess(userId) {
    ...
    const modules = await userPermissionRepository.findModules({
      where: { id: roleModuleIds },
      include: [
        {
          association: "modulePermissions",
    ...
    for (const up of userPermissions) {
      ...
      if (!userAccess[mp.moduleId]) {
        userAccess[mp.moduleId] = [];
      }

      userAccess[mp.moduleId].push(mp.permission.action);
    }
  ...
  async buildSidebarMenu(userId) {
    ...
    const modules = matrix.modules.filter((m) => roleModuleIds.includes(m.id));
  ...
  applyUserPermissionRules(modules, userPermissions) {
    ...
    for (const m of modules) {
      ...
      const userHasAny = configuredPermissionIds.some((id) => userPermissionSet.has(id));
    ...
    for (const m of Object.values(moduleMap)) {
      ...
      if (!parentId) {
        tree[m.id] = m;
      } else if (moduleMap[parentId]) {
        moduleMap[parentId].children.push(m);
      }
    }
